...
This document explains how Unity clients query the memory system and how Gemini processes those queries.

---

## Overview

The Unity app connects via WebSocket to `/ws/unity/{user_id}` and sends natural language queries about the user's memories. Gemini interprets these queries, fetches relevant data, and returns helpful responses.
...
|-------|----------|-------------|
| `type` | Yes | Must be `"query"` |
| `text` | Yes | Natural language query |
| `dateRange` | No | Limit search to date range |
| `includeFaces` | No | Whether to include face images (default: true) |
